...
document.getElementById('pengaduanWA').addEventListener('click', function(e) {
  ...
  window.open(`https://wa.me/088210749273${waNumber}?text=${encodeURIComponent(text)}`, '_blank');
});

// === PENGADUAN VIA EMAIL (EmailJS) ===
document.getElementById('pengaduanEmail').addEventListener('click', function(e) {
  e.preventDefault();
  
  // Tampilkan form modal sederhana (bisa diganti dengan form yang lebih bagus)
  const nama = prompt("Masukkan nama Anda:");
  if (!nama) return;
  
  const email = prompt("Masukkan email Anda untuk konfirmasi:");
  if (!email) return;
  
  const pesan = prompt("Tuliskan pengaduan Anda:");
  if (!pesan) return;
  ...
  const templateParams = {
    ...
    from_email: email,
    message: pesan,
    to_name: "Paris Teknik Air Conditioner",
    reply_to: email
  };
  ...
  emailjs.send('YOUR_SERVICE_ID', 'YOUR_TEMPLATE_ID', templateParams)
    .then(function(response) {
      alert('Pengaduan berhasil dikirim! Kami akan segera merespon ke email Anda.');
      console.log('SUCCESS!', response.status, response.text);
    }, function(error) {
      alert('Gagal mengirim pengaduan. Silakan coba lagi atau hubungi via WhatsApp.');
...
// === Alternatif: Form Pengaduan Modal (Lebih Baik) ===
function showPengaduanForm(type) {
  // Buat modal sederhana (bisa diganti dengan desain yang lebih baik)
...
modal.innerHTML = `
  <div style="background: white; padding: 2rem; border-radius: 1.5rem; max-width: 500px; width: 90%; max-height: 90vh; overflow-y: auto;">
    <h3 style="color: var(--primary); margin-bottom: 1.5rem; display: flex; align-items: center; gap: 0.5rem;">
      ...
      Form Pengaduan ${type === 'email' ? '(Email)' : '(WhatsApp)'}
    </h3>
    ...
    <div class="form-group">
      <label><i class="${type === 'email' ? 'fas fa-envelope' : 'fab fa-whatsapp'}" style="color: var(--primary); margin-right: 0.5rem;"></i> ${type === 'email' ? 'Email *' : 'No. WhatsApp *'}</label>
      <input type="${type === 'email' ? 'email' : 'tel'}" id="modal_kontak" placeholder="${type === 'email' ? 'email@contoh.com' : '08123456789'}" style="border-left: 3px solid var(--primary);">
    </div>
    ...
    <div style="display: flex; gap: 1rem; margin-top: 2rem;">
      <button class="btn-primary" id="modal_kirim" style="flex: 2; background-color: ${type === 'email' ? 'var(--primary)' : '#25D366'};">
        <i class="${type === 'email' ? 'fas fa-paper-plane' : 'fab fa-whatsapp'}"></i> 
        Kirim via ${type === 'email' ? 'Email' : 'WhatsApp'}
      </button>
...
document.getElementById('modal_kirim').addEventListener('click', () => {
  ...
  if (type === 'email') {
    ...
    if (!kontak.includes('@') || !kontak.includes('.')) {
    ...
    // Tampilkan loading
    const kirimBtn = document.getElementById('modal_kirim');
    ...
    emailjs.send('YOUR_SERVICE_ID', 'YOUR_TEMPLATE_ID', templateParams)
      ...
      .catch((error) => {
        alert('❌ Gagal mengirim. Silakan coba lagi atau hubungi via WhatsApp.');
...
// Update event listener untuk menggunakan modal
document.getElementById('pengaduanWA').addEventListener('click', function(e) {
  e.preventDefault();
  showPengaduanForm('wa');
});

document.getElementById('pengaduanEmail').addEventListener('click', function(e) {
  e.preventDefault();
  showPengaduanForm('email');
});